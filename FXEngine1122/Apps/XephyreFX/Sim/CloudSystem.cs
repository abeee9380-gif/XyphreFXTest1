...
public sealed class CloudSystem
{
    ...
    private Vec2? _lastAnchor;

    public void Tick(double dt, bool active, Vec2 blobCenter, double blobRadius)
    {
        // Dragging the anchor used to instantly orphan every existing cloud (sudden distance
        // jump tripped the despawn check below), causing them all to vanish and respawn fresh.
        // Now the clouds move together with the anchor instead.
        if (_lastAnchor.HasValue)
        {
            double dx = blobCenter.X - _lastAnchor.Value.X;
            double dy = blobCenter.Y - _lastAnchor.Value.Y;
            if (dx != 0 || dy != 0)
            {
                foreach (var c in _clouds) { c.X += dx; c.Y += dy; }
            }
        }
        _lastAnchor = blobCenter;
