...
public partial class SettingsWindow : Window
{
    ...
    public SettingsWindow(SharedRuntime runtime, SceneWindow sceneWindow)
    {
        ...
        WeatherProviderCombo.ItemsSource = Enum.GetValues(typeof(WeatherApiProvider));
    ...
    private void RefreshSelectedElementPanel()
    {
        ...
        if (key is null)
        {
            ...
            SelectedElementLabel.Background = Brushes.Transparent;
            SelectedOffsetXBox.Text = "";
            SelectedOffsetYBox.Text = "";
        ...
        SelectedElementLabel.Background = new SolidColorBrush(Color.FromArgb(60, 168, 85, 247));
        ...
        if (key.StartsWith(CustomKeyPrefix))
        {
            ...
            SelectedOffsetXBox.Text = el.OffsetX.ToString();
            SelectedOffsetYBox.Text = el.OffsetY.ToString();
        ...
        {
            var c = _runtime.Config.Current.Celestial;
            SelectedElementLabel.Text = "Selected: Sun/moon";
            SelectedOffsetXBox.Text = c.OffsetX.ToString();
            SelectedOffsetYBox.Text = c.OffsetY.ToString();
            SelectedColorBox.Text = "";
            SelectedScaleBox.Text = c.Scale.ToString();
            SelectedFontCombo.SelectedItem = null;
            SelectedLineLengthBox.Text = "";
            SelectedLineThicknessBox.Text = "";
        }
        ...
        {
            var a = _runtime.Config.Current.CloudAnchor;
            SelectedElementLabel.Text = "Selected: Cloud spawn point";
            SelectedOffsetXBox.Text = a.OffsetX.ToString();
            SelectedOffsetYBox.Text = a.OffsetY.ToString();
            SelectedColorBox.Text = "";
            SelectedScaleBox.Text = "";
            SelectedFontCombo.SelectedItem = null;
            SelectedLineLengthBox.Text = "";
        ...
        else
        {
            ...
            SelectedOffsetXBox.Text = cfg.OffsetX.ToString();
            SelectedOffsetYBox.Text = cfg.OffsetY.ToString();
    ...
    private void OnApplySelectedClick(object? sender, RoutedEventArgs e)
    {
        ...
        if (key.StartsWith(CustomKeyPrefix))
        {
            ...
            if (double.TryParse(SelectedOffsetXBox.Text, out var ox)) el.OffsetX = ox;
            if (double.TryParse(SelectedOffsetYBox.Text, out var oy)) el.OffsetY = oy;
        ...
        {
            var c = _runtime.Config.Current.Celestial;
            if (double.TryParse(SelectedOffsetXBox.Text, out var ox)) c.OffsetX = ox;
            if (double.TryParse(SelectedOffsetYBox.Text, out var oy)) c.OffsetY = oy;
            if (double.TryParse(SelectedScaleBox.Text, out var scale) && scale > 0) c.Scale = scale;
        }
        else if (key == "CloudAnchor")
        {
            var a = _runtime.Config.Current.CloudAnchor;
            if (double.TryParse(SelectedOffsetXBox.Text, out var ox)) a.OffsetX = ox;
            if (double.TryParse(SelectedOffsetYBox.Text, out var oy)) a.OffsetY = oy;
        }
        else
        {
            var cfg = GetTextElementConfig(key);
            if (double.TryParse(SelectedOffsetXBox.Text, out var ox)) cfg.OffsetX = ox;
            if (double.TryParse(SelectedOffsetYBox.Text, out var oy)) cfg.OffsetY = oy;
    ...
    private async void OnEnableWeatherApiClick(object? sender, RoutedEventArgs e)
    {
        var cfg = _runtime.Config.Current.WeatherApi;
        cfg.Enabled = true;
        cfg.ApiKey = WeatherApiKeyBox.Text ?? "";
        if (WeatherProviderCombo.SelectedItem is WeatherApiProvider p) cfg.Provider = p;
        cfg.City = WeatherCityBox.Text ?? "";
        _runtime.Config.Save();
        await _sceneWindow.RefreshLiveWeatherAsync();
    }

    private void OnDisableWeatherApiClick(object? sender, RoutedEventArgs e)
    {
        _runtime.Config.Current.WeatherApi.Enabled = false;
        _runtime.Config.Save();
    }
    ...
    private void PopulateSettingsFields()
    {
        ...
        WeatherApiKeyBox.Text = cfg.WeatherApi.ApiKey;
        WeatherProviderCombo.SelectedItem = cfg.WeatherApi.Provider;
        WeatherCityBox.Text = cfg.WeatherApi.City;
        RefreshCelestialImageStatus();
    }
}
