...
public sealed class AvaloniaSceneRenderer
{
    ...
    private void DrawCelestial(Canvas canvas, CelestialBody body, Vec2 blobCenter, CelestialConfig config, bool selectable)
    {
        ...
        // Padded well beyond the visual size -- a small sun/moon was too precise a target to
        // reliably click/drag otherwise.
        const double hitPad = 16;
        var bounds = new Rect(pos.X - radius - hitPad, pos.Y - radius - hitPad, radius * 2 + hitPad * 2, radius * 2 + hitPad * 2);
    ...
    private void DrawTextElement(Canvas canvas, Vec2 center, string key, string text, double baseFontSize, FontWeight weight, TextElementConfig config, FontFamily fallbackFontFamily)
    {
        ...
        const double textHitPad = 8;
        var bounds = new Rect(left - textHitPad, top - textHitPad, block.DesiredSize.Width + textHitPad * 2, block.DesiredSize.Height + textHitPad * 2);
    ...
    private void DrawCustomElements(Canvas canvas, Vec2 center, List<CustomElementConfig> elements)
    {
        foreach (var el in elements)
        {
            ...
            else
            {
                ...
                double size = 64 * (el.Scale <= 0 ? 1.0 : el.Scale);
            ...
            const double customHitPad = 10;
            bounds = new Rect(bounds.X - customHitPad, bounds.Y - customHitPad, bounds.Width + customHitPad * 2, bounds.Height + customHitPad * 2);
